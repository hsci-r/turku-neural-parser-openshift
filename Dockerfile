...
SHELL ["/bin/bash", "-c"]
WORKDIR /app
RUN apt-get update && apt-get install -y python3-tk && echo "MODELS: $models" && for m in ${models} ; do echo "DOWNLOADING $m" ; python3 fetch_models.py $m ; done && find /app -name "models_*" -type d -exec chmod a+rwx -R {} \;
